...
export const PRICE_CACHE_KEY = 'ul-market-price-cache-v32';
...
export function validateMarketPrice(apiPrice, name, category, wear, rarity) {
  category = detectCategory(category, name);
  const fallback = fallbackPrice(name, rarity, category, wear);
  const n = String(name).toLowerCase();
  let floor = .03;
  if (category === 'knife') floor = Math.max(15, fallback * .15);
  if (category === 'glove') floor = Math.max(10, fallback * .15);
