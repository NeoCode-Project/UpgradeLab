...
async function animateWheel(r){const spinStyle=localStorage.getItem('ul-spin-style')||'normal';const d=fast?1800:(spinStyle==='long'?8200:5200),turns=fast?4:10,final=r.angle??Math.random()*360,arc=$('#winArc'),ticks=$('.ticks');[arc,ticks].filter(Boolean).forEach(el=>el.getAnimations().forEach(a=>a.cancel()));const stopTicks=sound?startSpinTicks(d):()=>{};const keyframes=[{transform:'rotate(0deg)'},{transform:`rotate(${turns*360+final}deg)`}],options={duration:d,easing:'cubic-bezier(.12,.02,.08,1)',fill:'forwards'};const animations=[arc,ticks].filter(Boolean).map(el=>el.animate(keyframes,options));try{await Promise.all(animations.map(a=>a.finished))}finally{stopTicks()}if(sound)resultSound(r.win)}
