...
async function animateWheel(r){const spinStyle=localStorage.getItem('ul-spin-style')||'normal';const d=fast?1800:(spinStyle==='long'?8200:5200),turns=fast?4:10,final=r.angle??Math.random()*360,needle=$('#needle');if(!needle)return;if(needle._wheelAnimation){try{needle._wheelAnimation.cancel()}catch{}}needle.getAnimations().forEach(a=>a.cancel());const stopTicks=sound?startSpinTicks(d):()=>{};const current=Number(needle.dataset.angle||0);const end=current+turns*360+((final-current%360+360)%360);const animation=needle.animate([{transform:`rotate(${current}deg)`},{transform:`rotate(${end}deg)`}],{duration:d,easing:'cubic-bezier(.12,.02,.08,1)',fill:'forwards'});needle._wheelAnimation=animation;try{await animation.finished;needle.dataset.angle=String(end);needle.style.transform=`rotate(${end}deg)`}finally{stopTicks();needle._wheelAnimation=null}if(sound)resultSound(r.win)}
