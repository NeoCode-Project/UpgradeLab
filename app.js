...
const STEAM_CACHE_KEY='ul-steam-price-cache-v37';
...
async function enrichSteamPrices(items){
  const candidates=items.filter(x=>x&&x.priceSource!=='Steam Market — подтверждённая цена');
  ...
  for(let i=0;i<todo.length;i+=3){const batch=todo.slice(i,i+3);const prices=await Promise.all(batch.map(fetchSteamPrice));prices.forEach((p,j)=>{if(p>0){batch[j].priceUSD=p;batch[j].priceSource='Steam Market — подтверждённая цена';changed=true}})}
