...
function card(x,mode='catalog'){
  ...
  return `<article class="skin-card ${mode}-card rarity-${esc(x.rarity||'mil-spec')} ${mode==='catalog'&&targetId===id?'target-selected':''}" data-id="${esc(id)}"><button class="fav-btn ${fav?'active':''}" title="${fav?'Убрать из избранного':'Добавить в избранное'}">${fav?'★':'☆'}</button><div class="skin-img"><img src="${esc(x.image)}" loading="lazy"></div><span class="wear-badge">${esc(x.wear||'')}</span><h3>${esc(x.name)}</h3><small>${esc(x.wear||'')}</small><b>${fmt(x.priceUSD)}</b><em class="price-source">${esc(x.priceSource||'сохранённая цена')}</em>${mode==='catalog'?'<button class="choose compact-pick">＋</button>':mode==='inventory'?'<div class="card-actions"><button class="ghost use">Поставить</button><button class="ghost sell">Продать</button></div>':'<button class="ghost pick">Выбрать</button>'}</article>`
}
function filtered(){
...
$$('input[name="wheelStyle"]').forEach(r=>r.onchange=()=>{localStorage.setItem('ul-wheel-style',r.value);document.documentElement.dataset.wheel=r.value});
$$('[data-arrow]').forEach(b=>b.onclick=()=>{localStorage.setItem('ul-arrow-style',b.dataset.arrow);document.documentElement.dataset.arrow=b.dataset.arrow;applySettingsState()});
$$('input[name="spinStyle"]').forEach(r=>r.onchange=()=>localStorage.setItem('ul-spin-style',r.value));
$$('input[name="soundSetting"]').forEach(r=>r.onchange=()=>{sound=r.value==='on';localStorage.setItem('ul-sound',sound?'1':'0');renderTop()});const volumeInput=$('#soundVolume');if(volumeInput)volumeInput.oninput=()=>{soundVolume=Math.max(0,Math.min(1,Number(volumeInput.value)/100));localStorage.setItem('ul-sound-volume',String(soundVolume));if(sound)beep(260,.045,.16,'triangle')};
