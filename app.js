...
async function animateWheel(r){const spinStyle=localStorage.getItem('ul-spin-style')||'normal';const d=fast?1800:(spinStyle==='long'?8200:5200),turns=fast?4:10,final=r.angle??Math.random()*360,n=$('#needle');if(!n)return;if(n._wheelAnimation){try{n._wheelAnimation.cancel()}catch{}}n.getAnimations().forEach(a=>a.cancel());n.style.transform='rotate(0deg)';void n.offsetWidth;const stopTicks=sound?startSpinTicks(d):()=>{};const a=n.animate([{transform:'rotate(0deg)'},{transform:`rotate(${turns*360+final}deg)`}],{duration:d,easing:'cubic-bezier(.12,.02,.08,1)',fill:'forwards'});n._wheelAnimation=a;try{await a.finished}catch(e){if(e?.name!=='AbortError')throw e}finally{stopTicks();if(n._wheelAnimation===a)n._wheelAnimation=null}if(sound)resultSound(r.win)}
