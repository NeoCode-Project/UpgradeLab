...
async function activatePromo(){
  ...
  try{let rewardText='';await runTransaction(db,async tx=>{
    const pref=doc(db,'promocodes',code),uref=doc(db,'users',user.uid),ps=await tx.get(pref),us=await tx.get(uref);
    if(!ps.exists())throw new Error('Промокод не найден');const promo=ps.data()||{},d=us.data()||{};
    ...
    const usedCount=Number(promo.usedCount||0),max=Number(promo.maxUses||0);if(max&&usedCount>=max)throw new Error('Лимит активаций закончился');
    ...
    tx.update(pref,{usedCount:usedCount+1});
    if(xp||levels){const profileRef=doc(db,'profiles',user.uid),profileSnap=await tx.get(profileRef),pd=profileSnap.data()||{};tx.set(profileRef,{xp:Number(pd.xp||0)+xp,level:Math.max(1,Number(pd.level||1)+levels)},{merge:true})}
    rewardText=[money?fmt(money):'',xp?`+${xp} XP`:'',levels?`+${levels} ур.`:'',awarded.length?`${awarded.length} предмет(а)`:'' ].filter(Boolean).join(', ');
  });$('#promoCode').value='';toast('Промокод активирован: '+(rewardText||'награда получена'))}catch(e){toast(humanError(e))}
}
function renderAdminPanel(){const panel=$('#adminPanel');if(!panel)return;const admin=profile?.role==='admin';panel.classList.toggle('hidden',!admin);$('#adminNav')?.classList.toggle('hidden',!admin)}
async function createPromoFromAdmin(){if(profile?.role!=='admin')return toast('Нет доступа');const code=String($('#adminPromoCode').value||'').trim().toUpperCase();if(!/^[A-Z0-9_-]{3,32}$/.test(code))return toast('Код: 3–32 символа');const money=parseMoneyInput($('#adminPromoMoney').value)/rates[currency],xp=parseMoneyInput($('#adminPromoXp').value),levels=parseMoneyInput($('#adminPromoLevels').value),maxUses=parseMoneyInput($('#adminPromoMax').value),days=parseMoneyInput($('#adminPromoDays').value);try{await setDoc(doc(db,'promocodes',code),{code,active:true,moneyUSD:money,xp,levels,maxUses,usedCount:0,endsAt:days?Date.now()+days*86400000:0,items:[],createdBy:user.uid,createdAt:Date.now()},{merge:true});toast('Промокод '+code+' сохранён')}catch(e){toast(humanError(e))}}
...
$('#activatePromo')?.addEventListener('click',activatePromo);$('#createPromoBtn')?.addEventListener('click',createPromoFromAdmin);
