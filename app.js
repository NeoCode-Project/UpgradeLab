...
renderCrystalShop=function(){
 ...
 grid.innerHTML=products.map(p=>{const owned=v66OwnedText(p);const isOwned=p.type==='cosmetic'&&(userDoc?.cosmetics?.owned||[]).includes(p.key);const active=isOwned&&(userDoc?.cosmetics?.activeTheme||'')===p.key;const label=p.type==='case'?'Открыть':active?'Активно':isOwned?'Применить':'Купить';return `<article class="crystal-product v66-${p.type}"><div class="crystal-product-icon">${p.icon}</div><h3>${esc(p.title)}</h3><p>${esc(p.desc)}</p>${owned?`<small class="owned-label">${esc(owned)}</small>`:''}<b class="shop-price">${isOwned?'Куплено':`💎 ${p.cost}`}</b><button class="upgrade-btn" data-v66-buy="${p.id}" ${(count<p.cost&&!isOwned)||active?'disabled':''}>${label}</button></article>`}).join('');
...
/* V72 — mini Battle Pass */
const BATTLE_PASS_COST=100;
const BATTLE_PASS_LEVELS=[
 {free:{crystals:1},premium:{crystals:3}},
 {free:{moneyUSD:250},premium:{moneyUSD:1000}},
 {free:{crystals:1},premium:{crystals:4}},
 {free:{moneyUSD:500},premium:{moneyUSD:1500}},
 {free:{crystals:2},premium:{crystals:5}},
 {free:{moneyUSD:750},premium:{moneyUSD:2500}},
 {free:{crystals:2},premium:{crystals:6}},
 {free:{moneyUSD:1000},premium:{moneyUSD:3500}},
 {free:{crystals:3},premium:{crystals:8}},
 {free:{moneyUSD:2000,crystals:3},premium:{moneyUSD:5000,crystals:10}}
];
...
function renderBattlePass(){
 ...
 const bp=userDoc?.battlePass||{},xp=Math.max(0,Number(bp.xp||0)),level=Math.min(10,Math.floor(xp/100)+1),progress=level>=10?100:xp%100;
 ...
 $('#bpLevel').textContent=`Уровень ${level}`;$('#bpXpText').textContent=level>=10?`${xp} XP · максимум`:`${xp%100} / 100 XP`;$('#bpProgress').style.width=progress+'%';
 const buy=$('#buyBattlePass');buy.textContent=premium?'✓ Улучшенный пропуск открыт':`Улучшить за 💎 ${BATTLE_PASS_COST}`;buy.disabled=premium;
 box.innerHTML=BATTLE_PASS_LEVELS.map((rw,i)=>{const n=i+1,reached=xp>=i*100,fc=freeClaimed.has(n),pc=premiumClaimed.has(n);return `<article class="bp-level-card ${reached?'reached':''}"><div class="bp-level-head"><b>Уровень ${n}</b><span>${i*100} XP</span></div><div class="bp-reward ${!reached?'locked':''} ${fc?'claimed':''}"><small>Обычная награда</small><strong>${bpRewardText(rw.free)}</strong><button class="ghost" data-bp-claim="free:${n}" ${!reached||fc?'disabled':''}>${fc?'Получено':'Забрать'}</button></div><div class="bp-reward premium ${!reached||!premium?'locked':''} ${pc?'claimed':''}"><small>Улучшенная награда</small><strong>${bpRewardText(rw.premium)}</strong><button class="ghost" data-bp-claim="premium:${n}" ${!reached||!premium||pc?'disabled':''}>${pc?'Получено':premium?'Забрать':'Нужен пропуск'}</button></div></article>`}).join('');
...
async function buyBattlePass(){
 if(!user)return;try{await runTransaction(db,async tx=>{const ref=doc(db,'users',user.uid),snap=await tx.get(ref);if(!snap.exists())throw new Error('Профиль не найден');const d=snap.data()||{},bp=d.battlePass||{},cr=Number(d.crystals||0);if(bp.premium)throw new Error('Пропуск уже открыт');if(cr<BATTLE_PASS_COST)throw new Error('Недостаточно кристаллов');tx.update(ref,{crystals:cr-BATTLE_PASS_COST,battlePass:{...bp,premium:true}})});toast('🎫 Улучшенный Battle Pass открыт')}catch(e){toast(humanError(e))}
}
async function claimBattlePassReward(key){
 const [track,nRaw]=String(key).split(':'),n=Number(nRaw),reward=BATTLE_PASS_LEVELS[n-1]?.[track];if(!reward||!user)return;
 try{await runTransaction(db,async tx=>{const ref=doc(db,'users',user.uid),snap=await tx.get(ref);if(!snap.exists())throw new Error('Профиль не найден');const d=snap.data()||{},bp=d.battlePass||{},xp=Number(bp.xp||0);if(xp<(n-1)*100)throw new Error('Уровень ещё не открыт');if(track==='premium'&&bp.premium!==true)throw new Error('Нужен улучшенный пропуск');const field=track==='premium'?'claimedPremium':'claimedFree',claimed=Array.isArray(bp[field])?bp[field].map(Number):[];if(claimed.includes(n))throw new Error('Награда уже получена');tx.update(ref,{balanceUSD:Number(d.balanceUSD||0)+Number(reward.moneyUSD||0),crystals:Number(d.crystals||0)+Number(reward.crystals||0),battlePass:{...bp,[field]:[...claimed,n]}})});toast('Награда Battle Pass получена')}catch(e){toast(humanError(e))}
}
