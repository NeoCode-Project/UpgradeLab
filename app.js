...
function renderCatalog(){
  ...
  $$(`#skinGrid .skin-card`).forEach(e=>{
    ...
    const buy=e.querySelector('.buy-item');if(buy)buy.onclick=()=>buyItem(candidate);
...
async function buyItem(item){
  ...
  try{
    await runTransaction(db,async tx=>{
      ...
      const bought={...item,uid:crypto.randomUUID(),obtainedAt:Date.now(),purchasePriceUSD:price,purchaseSource:item.priceSource||'Рыночная цена'};
...
// V44 — Cases
const CASES=[
['budget','Budget Case','popular',0.5],['starter','Starter Case','popular',1],['lucky','Lucky Case','popular',2],['mystery','Mystery Case','popular',3.5],['neon','Neon Case','theme',5],['fire','Fire Case','theme',7.5],['ice','Ice Case','theme',7.5],['galaxy','Galaxy Case','theme',10],['dragon','Dragon Case','theme',14],['sakura','Sakura Case','theme',12],['lightning','Lightning Case','theme',16],['premium','Premium Case','premium',25],['diamond','Diamond Case','premium',40],['gold','Gold Case','premium',55],['emerald','Emerald Case','rare',70],['ruby','Ruby Case','rare',80],['sapphire','Sapphire Case','rare',90],['ak47','AK-47 Case','weapon',6],['m4','M4 Case','weapon',6],['awp','AWP Case','weapon',9],['deagle','Desert Eagle Case','weapon',4],['usp','USP-S Case','weapon',3],['glock','Glock Case','weapon',3],['knife','Knife Case','rare',120],['gloves','Gloves Case','rare',110],['milspec','Mil-Spec Case','popular',2.5],['restricted','Restricted Case','popular',8],['classified','Classified Case','premium',22],['covert','Covert Case','premium',60],['collector','Collector Case','premium',150],['elite','Elite Case','premium',500],['royal','Royal Case','premium',1500],['titan','Titan Case','rare',5000],['legend','Legend Case','rare',15000],['millionaire','Millionaire Case','rare',50000],['billion','Billion Dream Case','rare',150000]
].map((x,i)=>({slug:x[0],name:x[1],category:x[2],priceUSD:x[3],popularity:30-i,image:`assets/cases/${x[0]}.svg`}));
...
function casePool(c){let pool=skins.filter(x=>Number(x.priceUSD)>0&&x.image);if(c.category==='weapon')pool=pool.filter(x=>x.category==='weapon'&&new RegExp(c.slug==='ak47'?'AK-47':c.slug==='m4'?'M4A4|M4A1-S':c.slug==='awp'?'AWP':c.slug==='deagle'?'Desert Eagle':c.slug==='usp'?'USP-S':'Glock','i').test(x.name));if(c.slug==='knife')pool=pool.filter(x=>x.category==='knife');if(c.slug==='gloves')pool=pool.filter(x=>x.category==='glove');if(['milspec','restricted','classified','covert'].includes(c.slug))pool=pool.filter(x=>x.rarity===({'milspec':'mil-spec',restricted:'restricted',classified:'classified',covert:'covert'}[c.slug]));if(!pool.length)pool=skins.filter(x=>Number(x.priceUSD)>0&&x.image);if(c.priceUSD>=500){const base=[...pool].sort((a,b)=>b.priceUSD-a.priceUSD).slice(0,24);const tiers=[.08,.18,.35,.7,1.05,1.8,4,10,25];pool=tiers.flatMap((m,ti)=>base.slice(0,Math.max(4,12-ti)).map((x,i)=>({...x,id:`${c.slug}-${ti}-${i}-${x.id||x.name}`,name:`${x.name} · ${['Потёртый','Обычный','Редкий','Элитный','Особый','Премиум','Джекпот','Легендарный','Мифический'][ti]}`,priceUSD:Math.max(.03,Math.round(c.priceUSD*m*(.82+(i%5)*.09)*100)/100),rarity:ti>=7?'contraband':ti>=5?'covert':ti>=3?'classified':x.rarity})));return pool.sort((a,b)=>a.priceUSD-b.priceUSD)}const max=Math.max(c.priceUSD*35,c.priceUSD+5),min=Math.max(.03,c.priceUSD*.05);let ranged=pool.filter(x=>x.priceUSD>=min&&x.priceUSD<=max);return (ranged.length>=8?ranged:pool).sort((a,b)=>a.priceUSD-b.priceUSD)}
function renderCases(){const grid=$('#casesGrid');if(!grid)return;let list=[...CASES],q=$('#caseSearch')?.value.trim().toLowerCase()||'',cat=$('#caseCategory')?.value||'all',sort=$('#caseSort')?.value||'default';if(q)list=list.filter(c=>c.name.toLowerCase().includes(q));if(cat!=='all')list=list.filter(c=>c.category===cat);if(sort==='priceAsc')list.sort((a,b)=>a.priceUSD-b.priceUSD);else if(sort==='priceDesc')list.sort((a,b)=>b.priceUSD-a.priceUSD);grid.innerHTML=list.map(c=>`<article class="case-card" data-case="${c.slug}"><span class="case-tag">${esc(({popular:'ПОПУЛЯРНЫЙ',weapon:'ОРУЖИЕ',rare:'РЕДКИЙ',theme:'ТЕМАТИЧЕСКИЙ',premium:'ПРЕМИУМ'})[c.category])}</span><img src="${c.image}" alt="${esc(c.name)}" onerror="this.onerror=null;this.src='assets/cases/premium.svg'"><div class="case-card-body"><h3>${esc(c.name)}</h3><div class="case-card-meta"><span>Открыть кейс</span><span class="case-card-price">${fmt(c.priceUSD)}</span></div></div></article>`).join('')||'<div class="case-empty">Кейсы не найдены</div>';$$('.case-card').forEach(e=>e.onclick=()=>openCase(CASES.find(c=>c.slug===e.dataset.case)));if($('#casesBalance'))$('#casesBalance').textContent=fmt(userDoc?.balanceUSD||0)}
...
function openCase(c){if(!c)return toast('Кейс не найден. Обнови страницу.');activeCase=c;caseQty=1;caseAbortRequested=false;const pool=casePool(c),preview=stableCasePreview(c,pool);$('#caseModalContent').innerHTML=`<div class="case-open-layout"><div class="case-open-cover"><img src="${c.image}" alt="${esc(c.name)}" onerror="this.onerror=null;this.src='assets/cases/premium.svg'"></div><div class="case-open-info"><span class="eyebrow">${c.category.toUpperCase()}</span><h2>${esc(c.name)}</h2><p class="muted">Выбери количество кейсов. Максимум — 10 за одно открытие.</p><div class="case-open-price">${fmt(c.priceUSD)} за кейс</div><div class="case-qty">${Array.from({length:10},(_,i)=>i+1).map(n=>`<button class="ghost ${n===1?'active':''}" data-case-qty="${n}">×${n}</button>`).join('')}</div><div class="case-speed-row"><span>Скорость анимации</span><div class="case-speed-buttons">${[['fast','Быстро'],['normal','Обычно'],['slow','Медленно']].map(([v,t])=>`<button class="ghost ${caseSpinSpeed===v?'active':''}" data-case-speed="${v}">${t}</button>`).join('')}</div></div><div class="case-open-summary"><span>Итого</span><b id="caseOpenTotal">${fmt(c.priceUSD)}</b></div><button id="openCaseBtn" class="upgrade-btn">Открыть ×1</button></div></div><div class="case-contents"><h3>Что может выпасть</h3><div class="case-contents-grid">${preview.map(x=>`<div class="case-content-item"><img src="${esc(x.image)}"><b>${esc(x.name)}</b><small>${fmt(x.priceUSD)}</small></div>`).join('')}</div></div>`;$$('[data-case-qty]').forEach(b=>b.onclick=()=>{caseQty=Number(b.dataset.caseQty);$$('[data-case-qty]').forEach(x=>x.classList.toggle('active',x===b));$('#caseOpenTotal').textContent=fmt(c.priceUSD*caseQty);$('#openCaseBtn').textContent=`Открыть ×${caseQty}`});$$('[data-case-speed]').forEach(b=>b.onclick=()=>{caseSpinSpeed=b.dataset.caseSpeed;localStorage.setItem('ul-case-speed',caseSpinSpeed);$$('[data-case-speed]').forEach(x=>x.classList.toggle('active',x===b))});$('#openCaseBtn').onclick=openSelectedCase;open('caseModal')}
function weightedDrop(pool,c){if(!Array.isArray(pool)||!pool.length){pool=skins.filter(x=>Number(x.priceUSD)>0&&x.image)}if(!pool.length)throw new Error('Для этого кейса пока нет предметов');const r=Math.random();let mult;if(r<.0008)mult=25;else if(r<.006)mult=10;else if(r<.025)mult=4;else if(r<.08)mult=1.8;else if(r<.20)mult=1.05;else if(r<.42)mult=.70;else if(r<.72)mult=.35;else mult=.12;const wanted=c.priceUSD*mult,cands=pool.slice().sort((a,b)=>Math.abs(a.priceUSD-wanted)-Math.abs(b.priceUSD-wanted));const limit=Math.max(1,Math.min(4,cands.length)),picked=cands[Math.floor(Math.random()*limit)]||pool[0];return {...picked,uid:(crypto.randomUUID?crypto.randomUUID():`${Date.now()}-${Math.random()}`),obtainedAt:Date.now(),fromCase:c.name}}
function buildCaseReel(pool,winner){const targetIndex=26;return Array.from({length:31},(_,i)=>i===targetIndex?winner:pool[Math.floor(Math.random()*pool.length)])}
async function animateCaseDrop(pool,drop,index,total){const reel=buildCaseReel(pool,drop),targetIndex=26,durations={fast:[850,260],normal:[2600,650],slow:[5200,1100]},[spinMs,pauseMs]=durations[caseSpinSpeed]||durations.normal;$('#caseModalContent').innerHTML=`<div class="case-spin-head"><div><span class="eyebrow">ОТКРЫТИЕ КЕЙСОВ</span><h2>${esc(activeCase.name)}</h2></div><b>${index+1} / ${total}</b></div><div class="case-spinner"><div id="caseSpinnerTrack" class="case-spinner-track" style="transition-duration:${spinMs}ms">${reel.map((x,i)=>`<div class="spin-item ${i===targetIndex?'spin-winner':''}"><img src="${esc(x.image)}"><small>${esc(x.name)}</small><b>${fmt(x.priceUSD)}</b></div>`).join('')}</div></div><div id="caseSpinResult" class="case-spin-result hidden"><img src="${esc(drop.image)}"><div><small>Выпало</small><strong>${esc(drop.name)}</strong><b>${fmt(drop.priceUSD)}</b></div></div><p class="muted case-spin-status">Кейс ${index+1} из ${total} — рулетка крутится…</p><p class="case-close-hint">Можно закрыть окно — все предметы автоматически попадут в инвентарь.</p>`;const spinner=$('.case-spinner'),track=$('#caseSpinnerTrack');await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));if(caseAbortRequested)return;const target=track.children[targetIndex];const spinnerCenter=spinner.clientWidth/2;const targetCenter=target.offsetLeft+target.offsetWidth/2;track.style.transform=`translateX(${spinnerCenter-targetCenter}px)`;await new Promise(r=>setTimeout(r,spinMs));if(caseAbortRequested)return;target.classList.add('landed');$('#caseSpinResult')?.classList.remove('hidden');const status=$('.case-spin-status');if(status)status.textContent=`Кейс ${index+1} из ${total} — выпало: ${drop.name}`;await new Promise(r=>setTimeout(r,pauseMs))}
async function addPendingCaseDropsToInventory(showToast=true){if(caseAutoSaving)return;const items=lastCaseDrops.filter(x=>!x.sold&&!x.kept);if(!items.length)return;caseAutoSaving=true;try{await runTransaction(db,async tx=>{const ref=doc(db,'users',user.uid),snap=await tx.get(ref),d=snap.data()||{},inv=Array.isArray(d.inventory)?d.inventory:[];tx.update(ref,{inventory:[...items.map(({selected,sold,kept,...x})=>x),...inv]})});items.forEach(x=>x.kept=true);if(showToast)toast(`${items.length} предметов автоматически добавлено в инвентарь`)}catch(e){toast(humanError(e))}finally{caseAutoSaving=false}}
async function openSelectedCase(){if(!activeCase||caseQty<1||caseQty>10||caseOpening)return;const cost=activeCase.priceUSD*caseQty;if(Number(userDoc?.balanceUSD||0)<cost)return toast('Недостаточно виртуального баланса');const btn=$('#openCaseBtn');btn.disabled=true;const pool=casePool(activeCase);const drops=Array.from({length:caseQty},()=>weightedDrop(pool,activeCase));const old=$('#caseModalContent').innerHTML;try{await runTransaction(db,async tx=>{const ref=doc(db,'users',user.uid),snap=await tx.get(ref),d=snap.data()||{},balance=Number(d.balanceUSD||0);if(balance<cost)throw new Error('Недостаточно виртуального баланса');const hist=Array.isArray(d.history)?d.history:[];const entry={id:crypto.randomUUID(),type:'case',caseName:activeCase.name,caseSlug:activeCase.slug,count:caseQty,costUSD:cost,drops:drops.map(x=>({name:x.name,image:x.image,priceUSD:x.priceUSD,rarity:x.rarity})),createdAt:Date.now()};tx.update(ref,{balanceUSD:balance-cost,history:[entry,...hist].slice(0,100)})});lastCaseDrops=drops.map(x=>({...x,selected:false,sold:false,kept:false}));caseOpening=true;caseAbortRequested=false;for(let i=0;i<drops.length&&!caseAbortRequested;i++)await animateCaseDrop(pool,drops[i],i,drops.length);caseOpening=false;if(caseAbortRequested){await addPendingCaseDropsToInventory(true);return}close('caseModal');renderCaseResults();open('caseResultModal')}catch(e){caseOpening=false;toast(humanError(e));$('#caseModalContent').innerHTML=old;const retry=$('#openCaseBtn');if(retry)retry.onclick=openSelectedCase}finally{if(btn)btn.disabled=false}}
...
$('#autoUpgradeStart')?.addEventListener('click',runAutoUpgrade);
$('#autoUpgradeStop')?.addEventListener('click',()=>{autoUpgradeRunning=false;setAutoUpgradeUI();toast('Автокрутка остановлена')});
setAutoUpgradeUI();

let contractSize=3,contractSelected=new Set(),contractRunning=false,contractDone=0,contractLimit=1;
function contractSelectedItems(){return inventory.filter(x=>contractSelected.has(inventoryKey(x))).slice(0,contractSize)}
function renderContracts(){
  ...
  $('#contractProgress').textContent=contractRunning?`Контрактов: ${contractDone}${contractLimit?` / ${contractLimit}`:' · до остановки'}`:`Выбрано ${chosen.length} из ${contractSize}`;
  $('#contractStart').disabled=contractRunning||chosen.length!==contractSize;$('#contractStop').disabled=!contractRunning;
  ...
  $$('#contractInventory .skin-card').forEach(el=>{
    ...
    const toggle=()=>{if(contractRunning)return;if(contractSelected.has(id))contractSelected.delete(id);else if(contractSelected.size<contractSize)contractSelected.add(id);else return toast(`Можно выбрать только ${contractSize} предметов`);renderContracts()};
...
$$('[data-contract-size]').forEach(b=>b.addEventListener('click',()=>{if(contractRunning)return;contractSize=Number(b.dataset.contractSize);contractSelected.clear();$$('[data-contract-size]').forEach(x=>x.classList.toggle('active',x===b));renderContracts()}));
$('#contractStart')?.addEventListener('click',startContracts);$('#contractStop')?.addEventListener('click',()=>{contractRunning=false;renderContracts();toast('Автоконтракт остановлен')});
