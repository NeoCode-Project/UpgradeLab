...
import { getFirestore, doc, getDoc, onSnapshot, collection, query, orderBy, limit, getDocs, setDoc, updateDoc, runTransaction, addDoc, deleteDoc, serverTimestamp } from 'https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js';
...
async function loadAdminPromos(){
  ...
  try{const snap=await getDocs(collection(db,'promocodes'));const rows=[];snap.forEach(d=>rows.push({id:d.id,...d.data()}));rows.sort((a,b)=>Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0));
    box.innerHTML=rows.length?rows.map(p=>`<div class="promo-row"><div><b>${esc(p.code||p.id)}</b><span class="promo-status ${p.active===false?'off':'on'}">${p.active===false?'выключен':'активен'}</span><small>${[p.moneyUSD?fmt(p.moneyUSD):'',p.xp?`+${p.xp} XP`:'',p.levels?`+${p.levels} ур.`:'',Array.isArray(p.items)&&p.items.length?`${p.items.length} предм.`:''].filter(Boolean).join(' · ')||'без награды'}</small>${Array.isArray(p.items)&&p.items.length?`<small>${p.items.map(x=>esc(x.name||'Предмет')).join(', ')}</small>`:''}</div><div class="promo-actions"><button type="button" data-edit-promo="${esc(p.code||p.id)}">Изменить</button><button type="button" class="danger" data-delete-promo="${esc(p.code||p.id)}">Удалить</button></div></div>`).join(''):'<span class="muted">Промокодов пока нет</span>';
