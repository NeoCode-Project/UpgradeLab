...
async function animateWheel(r){
  ...
  const finalAngle=Number(r.angle??Math.random()*360);
  const pointer=$('#pointerGroup');
  if(!pointer) throw new Error('Не найдена стрелка рулетки');
  const startAngle=0;
  const endAngle=turns*360+finalAngle;
  pointer.setAttribute('transform','rotate(0 165 165)');
  ...
  try{
    await new Promise(resolve=>{
      const frame=now=>{
        ...
        const angle=startAngle+(endAngle-startAngle)*ease(progress);
        pointer.setAttribute('transform',`rotate(${angle} 165 165)`);
