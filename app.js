...
import { getFirestore, doc, getDoc, onSnapshot, collection, query, orderBy, limit, getDocs, setDoc, updateDoc, runTransaction, addDoc, serverTimestamp } from 'https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js';
...
function renderProfile(){if(!profile||!user)return;renderBattlePass();const n=profile.nickname||'Игрок',st=profile.stats||{};$('#profileNick').textContent=n;$('#profileAvatar').textContent=avatarLetter(n);$('#profileUid').textContent='ID профиля: '+user.uid.slice(0,8);$('#nickInput').value=n;$('#statsGrid').innerHTML=`<div class="stat"><b>${st.spins||0}</b><small>апгрейдов</small></div><div class="stat"><b>${st.wins||0}</b><small>побед</small></div><div class="stat"><b>${st.losses||0}</b><small>поражений</small></div><div class="stat"><b>${fmt(st.bestWinUSD||0)}</b><small>лучший выигрыш</small></div>`}
...
// V71 Mini Battle Pass
const BP_LEVELS=10, BP_XP_PER_LEVEL=100, BP_PREMIUM_COST=100;
const BP_FREE=[
  {type:'crystals',amount:25,label:'25 кристаллов'},
  {type:'money',amount:25,label:'Бонус к балансу'},
  {type:'crystals',amount:15,label:'15 кристаллов'},
  {type:'money',amount:50,label:'Большой бонус'},
  {type:'crystals',amount:20,label:'20 кристаллов'},
  {type:'money',amount:75,label:'Бонус к балансу'},
  {type:'crystals',amount:25,label:'25 кристаллов'},
  {type:'money',amount:100,label:'Большой бонус'},
  {type:'crystals',amount:40,label:'40 кристаллов'},
  {type:'money',amount:200,label:'Финальная награда'}
];
const BP_PREMIUM=[
  {type:'money',amount:50,label:'Премиум-бонус'},
  {type:'crystals',amount:20,label:'20 кристаллов'},
  {type:'money',amount:80,label:'Премиум-бонус'},
  {type:'crystals',amount:30,label:'30 кристаллов'},
  {type:'money',amount:120,label:'Редкая награда'},
  {type:'crystals',amount:40,label:'40 кристаллов'},
  {type:'money',amount:160,label:'Премиум-бонус'},
  {type:'crystals',amount:50,label:'50 кристаллов'},
  {type:'money',amount:250,label:'Элитная награда'},
  {type:'crystals',amount:100,label:'100 кристаллов'}
];
function bpLevelFromProfile(p=profile){return Math.max(1,Math.min(BP_LEVELS,Number(p?.level||1),Math.floor(Number(p?.xp||0)/BP_XP_PER_LEVEL)+1))}
function bpEffectiveLevel(p=profile){return Math.max(Number(p?.level||1),Math.floor(Number(p?.xp||0)/BP_XP_PER_LEVEL)+1)}
function rewardIcon(r){return r.type==='crystals'?'💎':'₴'}
function renderBattlePass(){
  if(!profile)return;
  const xp=Math.max(0,Number(profile.xp||0)),level=bpEffectiveLevel(),shown=Math.min(BP_LEVELS,level),progress=level>=BP_LEVELS?100:(xp%BP_XP_PER_LEVEL)/BP_XP_PER_LEVEL*100,crystals=Math.max(0,Number(profile.crystals||0));
  $('#bpLevel').textContent=shown;$('#bpXp').textContent=level>=BP_LEVELS?'MAX':`${xp%BP_XP_PER_LEVEL} / ${BP_XP_PER_LEVEL}`;$('#bpCrystals').textContent=`${crystals} 💎`;$('#topCrystals').textContent=crystals;$('#bpProgressFill').style.width=`${progress}%`;
  const premium=Boolean(profile.battlePassPremium),freeClaimed=new Set((profile.bpFreeClaimed||[]).map(Number)),premiumClaimed=new Set((profile.bpPremiumClaimed||[]).map(Number));
  const buy=$('#buyBattlePass');buy.disabled=premium||crystals<BP_PREMIUM_COST;buy.textContent=premium?'Улучшенный пропуск активен':`Купить за ${BP_PREMIUM_COST} 💎`;buy.classList.toggle('bp-owned',premium);
  $('#battlePassGrid').innerHTML=Array.from({length:BP_LEVELS},(_,i)=>{const lvl=i+1,unlocked=level>=lvl,fr=BP_FREE[i],pr=BP_PREMIUM[i],fc=freeClaimed.has(lvl),pc=premiumClaimed.has(lvl);return `<article class="bp-level-card"><div class="bp-level-head"><b>Уровень ${lvl}</b><span>${unlocked?'✓':'🔒'}</span></div><div class="bp-reward ${unlocked?'':'locked'}"><strong>${rewardIcon(fr)} ${esc(fr.label)}</strong><small>Обычная дорожка</small><button class="bp-claim" data-bp-track="free" data-bp-level="${lvl}" ${!unlocked||fc?'disabled':''}>${fc?'Получено':unlocked?'Забрать':'Нужен уровень'}</button></div><div class="bp-reward premium ${unlocked&&premium?'':'locked'}"><strong>${rewardIcon(pr)} ${esc(pr.label)}</strong><small>Улучшенная дорожка</small><button class="bp-claim" data-bp-track="premium" data-bp-level="${lvl}" ${!unlocked||!premium||pc?'disabled':''}>${pc?'Получено':!premium?'Нужен пропуск':unlocked?'Забрать':'Нужен уровень'}</button></div></article>`}).join('');
  $$('#battlePassGrid [data-bp-level]').forEach(b=>b.onclick=()=>claimBattlePassReward(b.dataset.bpTrack,Number(b.dataset.bpLevel)));
}
async function buyBattlePass(){try{await runTransaction(db,async tx=>{const ref=doc(db,'profiles',user.uid),s=await tx.get(ref),p=s.data()||{},c=Number(p.crystals||0);if(p.battlePassPremium)throw new Error('Пропуск уже куплен');if(c<BP_PREMIUM_COST)throw new Error('Недостаточно кристаллов');tx.set(ref,{crystals:c-BP_PREMIUM_COST,battlePassPremium:true},{merge:true})});toast('Улучшенный Battle Pass открыт')}catch(e){toast(humanError(e))}}
async function claimBattlePassReward(track,lvl){try{await runTransaction(db,async tx=>{const pref=doc(db,'profiles',user.uid),uref=doc(db,'users',user.uid),ps=await tx.get(pref),us=await tx.get(uref),p=ps.data()||{},u=us.data()||{};if(bpEffectiveLevel(p)<lvl)throw new Error('Уровень ещё не открыт');if(track==='premium'&&!p.battlePassPremium)throw new Error('Нужен улучшенный пропуск');const field=track==='premium'?'bpPremiumClaimed':'bpFreeClaimed',claimed=Array.isArray(p[field])?p[field].map(Number):[];if(claimed.includes(lvl))throw new Error('Награда уже получена');const reward=(track==='premium'?BP_PREMIUM:BP_FREE)[lvl-1];const patch={[field]:[...claimed,lvl]};if(reward.type==='crystals')patch.crystals=Number(p.crystals||0)+reward.amount;tx.set(pref,patch,{merge:true});if(reward.type==='money')tx.set(uref,{balanceUSD:Number(u.balanceUSD||0)+reward.amount},{merge:true})});toast('Награда получена')}catch(e){toast(humanError(e))}}
...
async function switchGameMode(next){
  ...
  await runTransaction(db,async tx=>{
    ...
    if(next==='sandbox'){
      ...
      const box=d.normalState||{balanceUSD:START_BALANCE_USD,inventory:[],history:[],favorites:[],infiniteMoney:false};
...
function renderAdminPromoItems(){
  ...
  box.innerHTML=adminPromoItems.length?adminPromoItems.map((x,i)=>`<div class="admin-item-chip"><img src="${x.image||''}" alt=""><span>${escapeHtml(x.name)}<small>${fmt(x.priceUSD||0)}</small></span><button type="button" data-remove-promo-item="${i}">×</button></div>`).join(''):'<span class="muted">Предметы не выбраны</span>';
...
function setupAdminItemPicker(){
  const input=$('#adminPromoItemSearch'),list=$('#adminItemOptions');if(!input||!list)return;
  const best=[...skins].sort((a,b)=>Number(b.priceUSD||0)-Number(a.priceUSD||0)).slice(0,1000);
  list.innerHTML=best.map((x,i)=>`<option value="${escapeHtml(x.name)}" data-index="${i}">${fmt(x.priceUSD||0)}</option>`).join('');
  input._promoItems=best;
}
function addAdminPromoItem(){
  const input=$('#adminPromoItemSearch');if(!input)return;
  const value=String(input.value||'').trim().toLowerCase();
  const pool=input._promoItems||skins;
  const item=pool.find(x=>String(x.name||'').toLowerCase()===value)||skins.find(x=>String(x.name||'').toLowerCase().includes(value));
  if(!item)return toast('Предмет не найден');
  adminPromoItems.push(promoItemForSave(item));input.value='';renderAdminPromoItems();
}
async function activatePromo(){
  if(isSandbox())return toast('Промокоды работают только в обычном режиме');
  const code=String($('#promoCode')?.value||'').trim().toUpperCase();if(!code)return toast('Введи промокод');
  try{let rewardText='';await runTransaction(db,async tx=>{
    const pref=doc(db,'promocodes',code),uref=doc(db,'users',user.uid),profileRef=doc(db,'profiles',user.uid);
    const [ps,us,profileSnap]=await Promise.all([tx.get(pref),tx.get(uref),tx.get(profileRef)]);
    if(!ps.exists())throw new Error('Промокод не найден');if(!us.exists())throw new Error('Профиль пользователя не найден');
    const promo=ps.data()||{},d=us.data()||{},pd=profileSnap.exists()?(profileSnap.data()||{}):{};
    if(d.gameMode==='sandbox')throw new Error('Промокоды недоступны в песочнице');if(promo.active===false)throw new Error('Промокод выключен');
    const now=Date.now(),ends=Number(promo.endsAt||0);if(ends&&now>ends)throw new Error('Срок промокода истёк');
    const used=d.promoRedemptions||{};if(used[code])throw new Error('Ты уже использовал этот промокод');
    const money=Number(promo.moneyUSD||0),xp=Number(promo.xp||0),levels=Number(promo.levels||0),items=Array.isArray(promo.items)?promo.items:[];
    const awarded=items.map(x=>({...x,uid:safeUUID(),obtainedAt:Date.now(),source:'promo'}));
    tx.update(uref,{balanceUSD:Number(d.balanceUSD||0)+money,inventory:[...awarded,...(Array.isArray(d.inventory)?d.inventory:[])].slice(0,500),promoRedemptions:{...used,[code]:Date.now()}});
    if(xp||levels)tx.set(profileRef,{xp:Number(pd.xp||0)+xp,level:Math.max(1,Number(pd.level||1)+levels)},{merge:true});
    rewardText=[money?fmt(money):'',xp?`+${xp} XP`:'',levels?`+${levels} ур.`:'',awarded.length?`${awarded.length} предмет(а)`:'' ].filter(Boolean).join(', ');
  });$('#promoCode').value='';toast('Промокод активирован: '+(rewardText||'награда получена'))}catch(e){console.error(e);toast(humanError(e))}
}
...
async function createPromoFromAdmin(){
  ...
  const money=parseMoneyInput($('#adminPromoMoney').value)/rates[currency],xp=parseMoneyInput($('#adminPromoXp').value),levels=parseMoneyInput($('#adminPromoLevels').value),maxUses=parseMoneyInput($('#adminPromoMax').value),days=parseMoneyInput($('#adminPromoDays').value);
  try{const old=await getDoc(doc(db,'promocodes',code));const oldData=old.exists()?old.data():{};await setDoc(doc(db,'promocodes',code),{code,active:$('#adminPromoActive')?.checked!==false,moneyUSD:money,xp,levels,maxUses,usedCount:Number(oldData.usedCount||0),endsAt:days?Date.now()+days*86400000:0,items:adminPromoItems,createdBy:oldData.createdBy||user.uid,createdAt:oldData.createdAt||Date.now(),updatedAt:Date.now()},{merge:true});toast('Промокод '+code+' сохранён');adminPromoItems=[];renderAdminPromoItems();await loadAdminPromos()}catch(e){console.error(e);toast(humanError(e))}
}
async function loadAdminPromos(){
  ...
  try{const snap=await getDocs(collection(db,'promocodes'));const rows=[];snap.forEach(d=>rows.push({id:d.id,...d.data()}));rows.sort((a,b)=>Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0));
    box.innerHTML=rows.length?rows.map(p=>`<div class="promo-row"><div><b>${escapeHtml(p.code||p.id)}</b><span class="promo-status ${p.active===false?'off':'on'}">${p.active===false?'выключен':'активен'}</span><small>${[p.moneyUSD?fmt(p.moneyUSD):'',p.xp?`+${p.xp} XP`:'',p.levels?`+${p.levels} ур.`:'',Array.isArray(p.items)&&p.items.length?`${p.items.length} предм.`:''].filter(Boolean).join(' · ')||'без награды'}</small></div><button type="button" data-edit-promo="${escapeHtml(p.code||p.id)}">Изменить</button></div>`).join(''):'<span class="muted">Промокодов пока нет</span>';
    box.querySelectorAll('[data-edit-promo]').forEach(b=>b.onclick=()=>{const p=rows.find(x=>(x.code||x.id)===b.dataset.editPromo);if(!p)return;$('#adminPromoCode').value=p.code||p.id;$('#adminPromoMoney').value=Math.round(Number(p.moneyUSD||0)*rates[currency]*100)/100;$('#adminPromoXp').value=Number(p.xp||0);$('#adminPromoLevels').value=Number(p.levels||0);$('#adminPromoMax').value=Number(p.maxUses||0);$('#adminPromoDays').value=p.endsAt?Math.max(0,Math.ceil((Number(p.endsAt)-Date.now())/86400000)):0;if($('#adminPromoActive'))$('#adminPromoActive').checked=p.active!==false;adminPromoItems=Array.isArray(p.items)?p.items.map(promoItemForSave):[];renderAdminPromoItems();$('#adminPromoCode').scrollIntoView({behavior:'smooth',block:'center'})});
  }catch(e){console.error(e);box.innerHTML='<span class="muted">Не удалось загрузить промокоды: '+escapeHtml(humanError(e))+'</span>'}
}
