...
function renderHistory(){
  ...
  const rows=[...upgrades,...contracts].sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0));
  const itemRow=x=>`<div class="hist-item"><img src="${esc(x?.image||'')}" alt=""><span>${esc(x?.name||'Предмет')}</span><b>${fmt(Number(x?.priceUSD||0))}</b></div>`;
  $('#historyList').innerHTML=rows.map(h=>{
    if(h._kind==='contract'){
      const src=Array.isArray(h.sources)?h.sources:[],result=h.result||{};
      return `<article class="history-card history-contract"><div class="history-type">🧪 Контракт · ${src.length} предмета</div><div class="history-side history-source-list">${src.map(itemRow).join('')||'<div class="empty-state compact">Нет данных о предметах</div>'}</div><div class="history-arrow">→</div><div class="history-side">${itemRow(result)}</div><div class="history-meta"><b>${fmt(Number(h.totalUSD||src.reduce((n,x)=>n+Number(x.priceUSD||0),0)))} → ${fmt(Number(result.priceUSD||0))}</b><strong class="${Number(result.priceUSD||0)>=Number(h.totalUSD||0)?'win':'lose'}">${Number(result.priceUSD||0)>=Number(h.totalUSD||0)?'Плюс':'Минус'}</strong></div></article>`;
    }
    if(h._kind==='case'){
      const drops=Array.isArray(h.drops)?h.drops:[];
      return `<article class="history-card history-case"><div class="history-type">📦 Кейс: ${esc(h.caseName||'Без названия')} · ×${Number(h.count||drops.length||1)}</div><div class="history-side"><div class="hist-cash"><b>${fmt(Number(h.costUSD||0))}</b><small>стоимость открытия</small></div></div><div class="history-arrow">→</div><div class="history-side history-source-list">${drops.map(itemRow).join('')||'<div class="empty-state compact">Нет данных о выпадениях</div>'}</div><div class="history-meta"><b>Выпало: ${fmt(drops.reduce((n,x)=>n+Number(x.priceUSD||0),0))}</b><strong class="${drops.reduce((n,x)=>n+Number(x.priceUSD||0),0)>=Number(h.costUSD||0)?'win':'lose'}">${drops.reduce((n,x)=>n+Number(x.priceUSD||0),0)>=Number(h.costUSD||0)?'Окуп':'Не окуп'}</strong></div></article>`;
    }
    const src=Array.isArray(h.sources)?h.sources:[];
    return `<article class="history-card"><div class="history-type">⚡ Апгрейд</div><div class="history-side">${src.length?src.map(itemRow).join(''):`<div class="hist-cash"><b>${fmt(Number(h.cashUSD||0))}</b><small>ставка из баланса</small></div>`}</div><div class="history-arrow">→</div><div class="history-side">${itemRow(h.target||{})}</div><div class="history-meta"><b>${Number(h.chance||0).toFixed(2)}%${Number.isFinite(Number(h.roll))?` · выпало ${Number(h.roll).toFixed(2)}`:''}</b><strong class="${h.win?'win':'lose'}">${h.win?'Выигрыш':'Проигрыш'}</strong></div></article>`;
...
async function adminGrantPlayer(){
 if(!isAdmin())return toast('Нет доступа');const uid=$('#adminGrantUid')?.value.trim();if(!uid)return toast('Укажи UID игрока');
 const money=Number($('#adminGrantMoney')?.value||0),crystals=Number($('#adminGrantCrystals')?.value||0),xp=Number($('#adminGrantXp')?.value||0),levels=Number($('#adminGrantLevels')?.value||0),premium=$('#adminGrantPremium')?.checked===true;
 if(!money&&!crystals&&!xp&&!levels&&!premium)return toast('Укажи, что выдать');
 try{await runTransaction(db,async tx=>{const ref=doc(db,'users',uid),snap=await tx.get(ref);if(!snap.exists())throw new Error('Игрок с таким UID не найден');const d=snap.data()||{},bp=d.battlePass||{};tx.update(ref,{balanceUSD:Math.max(0,Number(d.balanceUSD||0)+money),crystals:Math.max(0,Number(d.crystals||0)+crystals),battlePass:{...bp,xp:Math.max(0,Number(bp.xp||0)+xp+levels*BATTLE_PASS_XP_PER_LEVEL),premium:premium?true:bp.premium===true},lastAdminGrant:{by:user.uid,at:Date.now(),money,crystals,xp,levels,premium}})});toast('Награда выдана игроку')}catch(e){console.error(e);toast(humanError(e))}
}
function resetAdminGrant(){['#adminGrantUid','#adminGrantMoney','#adminGrantCrystals','#adminGrantXp','#adminGrantLevels'].forEach((q,i)=>{const e=$(q);if(e)e.value=i===0?'':'0'});if($('#adminGrantPremium'))$('#adminGrantPremium').checked=false}
$('#adminGrantBtn')?.addEventListener('click',adminGrantPlayer);$('#adminGrantReset')?.addEventListener('click',resetAdminGrant);
...
async function loadAdminPromos(){
  ...
  try{const snap=await getDocs(collection(db,'promocodes'));const rows=[];snap.forEach(d=>rows.push({id:d.id,...d.data()}));rows.sort((a,b)=>Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0));
    box.innerHTML=rows.length?rows.map(p=>`<div class="promo-row"><div><b>${esc(p.code||p.id)}</b><span class="promo-status ${p.active===false?'off':'on'}">${p.active===false?'выключен':'активен'}</span><small>${[p.moneyUSD?fmt(p.moneyUSD):'',p.crystals?`💎 ${p.crystals}`:'',p.battlePassXp?`${p.battlePassXp} XP`:'',p.battlePassLevels?`${p.battlePassLevels} ур.`:'',p.battlePassPremium?'PREMIUM':'',Array.isArray(p.items)&&p.items.length?`${p.items.length} предм.`:''].filter(Boolean).join(' · ')||'без награды'}</small>${Array.isArray(p.items)&&p.items.length?`<small>${p.items.map(x=>esc(x.name||'Предмет')).join(', ')}</small>`:''}</div><div class="promo-actions"><button type="button" data-edit-promo="${esc(p.code||p.id)}">Изменить</button><button type="button" class="danger" data-delete-promo="${esc(p.code||p.id)}">Удалить</button></div></div>`).join(''):'<span class="muted">Промокодов пока нет</span>';
...
function renderV65Rewards(){
  ...
  const box=$('#achievementsList');if(box){const claimed=userDoc?.achievements||{},st=profile?.stats||{};const progressFor=a=>{if(a.id==='first_spin')return [Math.min(1,Number(st.spins||0)),1];if(a.id==='first_win')return [Math.min(1,Number(st.wins||0)),1];if(a.id==='ten_wins')return [Math.min(10,Number(st.wins||0)),10];if(a.id==='first_knife')return [inventory.some(x=>x.category==='knife'||/knife/i.test(x.name||''))?1:0,1];if(a.id==='collector')return [Math.min(100000,inventory.reduce((q,x)=>q+Number(x.priceUSD||0),0)),100000];return [a.ok()?1:0,1]};box.innerHTML=`<div class="achievement-summary"><div><b>${Object.keys(claimed).length}/${V65_ACHIEVEMENTS.length}</b><small>Получено</small></div><div><b>${Number(st.wins||0)}</b><small>Побед</small></div><div><b>${inventory.length}</b><small>Предметов</small></div></div><div class="achievement-grid">`+V65_ACHIEVEMENTS.map((a,i)=>{const done=Boolean(claimed[a.id]),ready=!done&&a.ok(),[cur,max]=progressFor(a),pct=Math.max(0,Math.min(100,cur/max*100));return `<article class="achievement-card ${done?'done':ready?'ready':''}"><div class="achievement-icon">${['🎯','🏆','⚡','🔪','💼'][i]||'🏅'}</div><div class="achievement-copy"><b>${esc(a.title)}</b><small>${esc(a.desc)}</small><div class="achievement-progress"><i style="width:${pct}%"></i></div><span>${Math.floor(cur).toLocaleString('ru-RU')} / ${Math.floor(max).toLocaleString('ru-RU')}</span><strong>${fmt(a.moneyUSD)} + 💎 ${a.crystals}</strong></div><button data-claim-achievement="${a.id}" ${done||!ready?'disabled':''}>${done?'✓ Получено':ready?'Забрать':'В процессе'}</button></article>`}).join('')+`</div>`;box.querySelectorAll('[data-claim-achievement]').forEach(b=>b.onclick=()=>claimAchievement(b.dataset.claimAchievement));}
}
