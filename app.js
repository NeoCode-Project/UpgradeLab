...
function detectCategory(rawCategory,name=''){
  const text=(String(rawCategory||'')+' '+String(name||'')).toLowerCase();
  if(/agent|character|terrorist|counter-terrorist/.test(text))return 'agent';
  if(/keychain|charm/.test(text))return 'keychain';
  if(/sticker/.test(text))return 'sticker';
  if(/graffiti|spray/.test(text))return 'graffiti';
  if(/crate|case|capsule|package|souvenir/.test(text))return 'case';
  const knifeWords=/(knife|knives|bayonet|karambit|dagger|daggers|bowie|falchion|navaja|stiletto|talon|ursus|paracord|survival|skeleton|nomad|kukri|gut knife|flip knife|huntsman|classic knife)/;
  if(knifeWords.test(text)||text.includes('★'))return 'knife';
...
function normalizeExtraItem(x,i,forcedCategory,priceMap){
  ...
  const rarity=rarityOf(x),category=detectCategory(forcedCategory,name);
  const marketHash=name;
  const apiPrice=priceMap.get(marketHash);
  const checked=sanePrice(apiPrice,marketHash,category,'',rarity);
  return {id:`${forcedCategory}-${x.id||x.def_index||i}`,name,marketHash,wear:'',image,rarity,category,priceUSD:checked.price,priceSource:checked.source};
}
async function loadCatalog(){
    ...
    if(forcedCategory==='weapon'){
      raw.filter(x=>x.image&&x.name).forEach((x,i)=>{
        ...
        for(const wear of wears){
          const hash=`${baseName} (${wear})`,key=hash.toLowerCase();if(seen.has(key))continue;seen.add(key);
          const category=detectCategory(String(x.category?.name||x.weapon?.name||''),baseName);
          const checked=sanePrice(priceMap.get(hash),hash,category,wear,rarity);
  ...
  $('#status').textContent=skins.length?`${skins.length} предметов: оружие ${counts.weapon+counts.knife+counts.glove}, агенты ${counts.agent}, брелоки ${counts.keychain}, наклейки ${counts.sticker}, граффити ${counts.graffiti}, кейсы ${counts.case}. Цены: ${priceResult.source}.`:'Каталог не загрузился';
  renderCatalog();
}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){const m=document.querySelector('.modal:not(.hidden)');if(m)close(m.id)}});$('#currency').onchange=e=>{currency=e.target.value;localStorage.setItem('ul-currency',currency);renderAll();renderCatalog()};$('#search').oninput=()=>{shown=30;renderCatalog()};$('#category').onchange=renderCatalog;$('#sort').onchange=renderCatalog;$('#more').onclick=()=>{shown+=30;renderCatalog()};$('#favoriteOnly').onclick=()=>{$('#favoriteOnly').classList.toggle('active');shown=30;renderCatalog()};$('#clearTarget').onclick=()=>{target=null;lastOutcome=null;syncUpgrade()};$('#clearSource').onclick=()=>{sourceItems=[];selected.clear();cashDraft='';lastOutcome=null;syncUpgrade()};$('#sourceMode').onclick=()=>{sourceItems=[];selected.clear();cashDraft='';lastOutcome=null;syncUpgrade()};$('#chooseInventory').onclick=()=>{selected=new Set(sourceItems.map(inventoryKey));renderPicker();open('modal')};$('#applySources').onclick=()=>{const picked=inventory.filter(x=>selected.has(inventoryKey(x)));if(!picked.length)return toast('Сначала выбери хотя бы один скин');sourceItems=picked;cashDraft='';lastOutcome=null;target=null;close('modal');syncUpgrade();toast(`В ставку добавлено: ${picked.length}`)};$('#upgradeBtn').onclick=startUpgrade;$('#autoTarget').onclick=()=>pickByRatio(2);function bindQuickButtons(){ $$('.quick > button[data-m],.quick > button[data-p]').forEach(b=>b.onclick=()=>b.dataset.m?pickByRatio(+b.dataset.m):openChanceTargets(+b.dataset.p)); } bindQuickButtons();$('#sellAll').onclick=()=>inventory.length?sellItems(inventory.map(inventoryKey)):toast('Инвентарь пуст');$('#soundQuick').onclick=e=>{e.preventDefault();e.stopPropagation();sound=!sound;localStorage.setItem('ul-sound',sound?'1':'0');renderTop();toast(sound?'Звук включён':'Звук выключен')};$('#fastQuick').onclick=e=>{e.preventDefault();e.stopPropagation();fast=!fast;localStorage.setItem('ul-fast',fast?'1':'0');renderTop();toast(fast?'Быстрый режим включён':'Быстрый режим выключен')};$('#saveNick').onclick=async()=>{try{const nickname=$('#nickInput').value.trim().slice(0,24)||'Игрок';await setDoc(doc(db,'profiles',user.uid),{nickname},{merge:true});toast('Ник сохранён')}catch(e){toast(humanError(e))}};
...
function applySettingsState(){
  const ws=localStorage.getItem('ul-wheel-style')||'bubble', ar=localStorage.getItem('ul-arrow-style')||'1', ss=localStorage.getItem('ul-spin-style')||'normal';
  document.documentElement.dataset.wheel=ws; document.documentElement.dataset.arrow=ar;
  const wr=document.querySelector(`input[name="wheelStyle"][value="${ws}"]`); if(wr)wr.checked=true;
  const sr=document.querySelector(`input[name="spinStyle"][value="${ss}"]`); if(sr)sr.checked=true;
  const snd=document.querySelector(`input[name="soundSetting"][value="${sound?'on':'off'}"]`); if(snd)snd.checked=true; const vol=$('#soundVolume');if(vol)vol.value=Math.round(soundVolume*100);
  $$('[data-arrow]').forEach(b=>b.classList.toggle('active',b.dataset.arrow===ar));
  $('#quickMultipliers').value=localStorage.getItem('ul-quick-m')||'2,3,5';
  $('#quickPercents').value=localStorage.getItem('ul-quick-p')||'20,40,70';
}
function rebuildQuickButtons(){
  const m=(localStorage.getItem('ul-quick-m')||'2,3,5').split(',').map(x=>Number(x.trim())).filter(x=>x>1).slice(0,3);
  const p=(localStorage.getItem('ul-quick-p')||'20,40,70').split(',').map(x=>Number(x.trim())).filter(x=>x>0&&x<=MAX_CHANCE).slice(0,3);
  const q=$('.quick'); if(!q)return; const custom=q.querySelector('.custom-chance');
  q.querySelectorAll(':scope > button[data-m],:scope > button[data-p]').forEach(x=>x.remove());
  [...m.map(v=>({m:v,label:`x${v}`})),...p.map(v=>({p:v,label:`${v}%`}))].forEach(o=>{const b=document.createElement('button'); if(o.m)b.dataset.m=o.m; else b.dataset.p=o.p; b.textContent=o.label; q.insertBefore(b,custom)});
  bindQuickButtons();
}
$('#themeQuick').onclick=e=>{e.preventDefault();e.stopPropagation();applySettingsState();open('themeModal')};
$$('.theme-grid button[data-theme]').forEach(b=>b.onclick=()=>setTheme(b.dataset.theme));
$$('input[name="wheelStyle"]').forEach(r=>r.onchange=()=>{localStorage.setItem('ul-wheel-style',r.value);document.documentElement.dataset.wheel=r.value;applySettingsState();toast(r.value==='classic'?'Старый круг включён':'Новый круг включён')});
$$('[data-arrow]').forEach(b=>b.onclick=e=>{e.preventDefault();localStorage.setItem('ul-arrow-style',b.dataset.arrow);document.documentElement.dataset.arrow=b.dataset.arrow;applySettingsState();toast('Стрелка изменена')});
$$('input[name="spinStyle"]').forEach(r=>r.onchange=()=>{localStorage.setItem('ul-spin-style',r.value);toast(r.value==='long'?'Длинная прокрутка включена':'Обычная прокрутка включена')});
$$('input[name="soundSetting"]').forEach(r=>r.onchange=()=>{sound=r.value==='on';localStorage.setItem('ul-sound',sound?'1':'0');renderTop();toast(sound?'Звук включён':'Звук выключен')});const volumeInput=$('#soundVolume');if(volumeInput)volumeInput.oninput=()=>{soundVolume=Math.max(0,Math.min(1,Number(volumeInput.value)/100));localStorage.setItem('ul-sound-volume',String(soundVolume));if(sound)beep(260,.045,.16,'triangle')};
$('#saveQuickSettings').onclick=()=>{localStorage.setItem('ul-quick-m',$('#quickMultipliers').value);localStorage.setItem('ul-quick-p',$('#quickPercents').value);rebuildQuickButtons();toast('Быстрые кнопки сохранены')};
rebuildQuickButtons();
...
/* V25: robust settings application. These final bindings intentionally override older handlers. */
function applyVisualSettingsV25({notify=false}={}){
  const theme=localStorage.getItem('ul-theme')||'classic';
  const wheelStyle=localStorage.getItem('ul-wheel-style')||'bubble';
  const arrowStyle=localStorage.getItem('ul-arrow-style')||'1';
  document.documentElement.dataset.theme=theme;
  document.body.dataset.theme=theme;
  document.documentElement.dataset.wheel=wheelStyle;
  document.body.dataset.wheel=wheelStyle;
  document.documentElement.dataset.arrow=arrowStyle;
  document.body.dataset.arrow=arrowStyle;
  document.querySelectorAll('.theme-grid button[data-theme]').forEach(btn=>btn.classList.toggle('active',btn.dataset.theme===theme));
  document.querySelectorAll('[data-arrow]').forEach(btn=>btn.classList.toggle('active',btn.dataset.arrow===arrowStyle));
  const wheelRadio=document.querySelector(`input[name="wheelStyle"][value="${wheelStyle}"]`);
  if(wheelRadio)wheelRadio.checked=true;
  const spinStyle=localStorage.getItem('ul-spin-style')||'normal';
  const spinRadio=document.querySelector(`input[name="spinStyle"][value="${spinStyle}"]`);
  if(spinRadio)spinRadio.checked=true;
  const soundRadio=document.querySelector(`input[name="soundSetting"][value="${sound?'on':'off'}"]`);
  if(soundRadio)soundRadio.checked=true;
  if(notify)toast('Настройки применены');
}

document.querySelectorAll('.theme-grid button[data-theme]').forEach(btn=>{
  btn.onclick=(event)=>{
    event.preventDefault();
    event.stopPropagation();
    localStorage.setItem('ul-theme',btn.dataset.theme);
    applyVisualSettingsV25();
    toast(`Тема: ${btn.textContent.trim()}`);
  };
});
document.querySelectorAll('input[name="wheelStyle"]').forEach(input=>{
  input.onchange=()=>{
    localStorage.setItem('ul-wheel-style',input.value);
    applyVisualSettingsV25();
    toast(input.value==='classic'?'Включён старый круг':'Включён новый круг');
  };
});
document.querySelectorAll('[data-arrow]').forEach(btn=>{
  btn.onclick=(event)=>{
    event.preventDefault();
    event.stopPropagation();
    localStorage.setItem('ul-arrow-style',btn.dataset.arrow);
    applyVisualSettingsV25();
    toast(`Стрелка ${btn.dataset.arrow} выбрана`);
  };
});
const settingsButtonV25=document.querySelector('#themeQuick');
if(settingsButtonV25)settingsButtonV25.onclick=(event)=>{
  event.preventDefault();
  event.stopPropagation();
  applySettingsState();
  applyVisualSettingsV25();
...
/* V27 authoritative wheel/settings bindings. */
function applyVisualSettingsV27(){
  const theme=localStorage.getItem('ul-theme')||'classic';
  const wheel=localStorage.getItem('ul-wheel-style')||'bubble';
  const arrow=localStorage.getItem('ul-arrow-style')||'1';
  for(const el of [document.documentElement,document.body]){
    el.dataset.theme=theme;
    el.dataset.wheel=wheel;
    el.dataset.arrow=arrow;
  }
  document.querySelectorAll('.theme-grid button[data-theme]').forEach(x=>x.classList.toggle('active',x.dataset.theme===theme));
  document.querySelectorAll('.arrow-options button[data-arrow]').forEach(x=>x.classList.toggle('active',x.dataset.arrow===arrow));
  const wr=document.querySelector(`input[name="wheelStyle"][value="${wheel}"]`); if(wr)wr.checked=true;
}
function bindSettingsV27(){
  document.querySelectorAll('.theme-grid button[data-theme]').forEach(btn=>{
    btn.onclick=e=>{e.preventDefault();e.stopPropagation();localStorage.setItem('ul-theme',btn.dataset.theme);applyVisualSettingsV27();toast(`Тема: ${btn.textContent.trim()}`)};
  });
  document.querySelectorAll('input[name="wheelStyle"]').forEach(input=>{
    input.onchange=()=>{localStorage.setItem('ul-wheel-style',input.value);applyVisualSettingsV27();toast(input.value==='classic'?'Старый круг включён':'Новый круг включён')};
  });
  document.querySelectorAll('.arrow-options button[data-arrow]').forEach(btn=>{
    btn.onclick=e=>{e.preventDefault();e.stopPropagation();localStorage.setItem('ul-arrow-style',btn.dataset.arrow);applyVisualSettingsV27();toast(`Стрелка ${btn.dataset.arrow} выбрана`)};
  });
  const settings=document.querySelector('#themeQuick');
  if(settings)settings.onclick=e=>{e.preventDefault();e.stopPropagation();applySettingsState();applyVisualSettingsV27();open('themeModal')};
}
bindSettingsV27();
applyVisualSettingsV27();


/* V28 authoritative settings + target-range picker. */
function applyAllSettingsV28(){
  const theme=localStorage.getItem('ul-theme')||'classic';
  const wheel=localStorage.getItem('ul-wheel-style')||'bubble';
  const arrow=localStorage.getItem('ul-arrow-style')||'1';
  const spinStyle=localStorage.getItem('ul-spin-style')||'normal';
  for(const el of [document.documentElement,document.body]){
    ...
    el.dataset.wheel=wheel;
    el.dataset.arrow=arrow;
    el.dataset.spin=spinStyle;
  }
  document.querySelectorAll('.theme-grid button[data-theme]').forEach(btn=>btn.classList.toggle('active',btn.dataset.theme===theme));
  document.querySelectorAll('.arrow-options button[data-arrow]').forEach(btn=>btn.classList.toggle('active',btn.dataset.arrow===arrow));
  const wr=document.querySelector(`input[name="wheelStyle"][value="${wheel}"]`); if(wr)wr.checked=true;
  const sr=document.querySelector(`input[name="spinStyle"][value="${spinStyle}"]`); if(sr)sr.checked=true;
  const snd=document.querySelector(`input[name="soundSetting"][value="${sound?'on':'off'}"]`); if(snd)snd.checked=true;
  const vol=document.querySelector('#soundVolume'); if(vol)vol.value=Math.round(soundVolume*100);
  const qm=document.querySelector('#quickMultipliers'); if(qm)qm.value=localStorage.getItem('ul-quick-m')||'2,3,5';
  const qp=document.querySelector('#quickPercents'); if(qp)qp.value=localStorage.getItem('ul-quick-p')||'20,40,70';
}
function bindAllSettingsV28(){
  document.querySelectorAll('.theme-grid button[data-theme]').forEach(btn=>btn.onclick=e=>{
    e.preventDefault();e.stopPropagation();localStorage.setItem('ul-theme',btn.dataset.theme);applyAllSettingsV28();toast(`Тема: ${btn.textContent.trim()}`);
  });
  document.querySelectorAll('input[name="wheelStyle"]').forEach(input=>input.onchange=()=>{
    localStorage.setItem('ul-wheel-style',input.value);applyAllSettingsV28();toast(input.value==='classic'?'Старый круг включён':'Новый круг включён');
  });
  document.querySelectorAll('.arrow-options button[data-arrow]').forEach(btn=>btn.onclick=e=>{
    e.preventDefault();e.stopPropagation();localStorage.setItem('ul-arrow-style',btn.dataset.arrow);applyAllSettingsV28();toast(`Стрелка ${btn.dataset.arrow} выбрана`);
  });
  document.querySelectorAll('input[name="spinStyle"]').forEach(input=>input.onchange=()=>{
    localStorage.setItem('ul-spin-style',input.value);applyAllSettingsV28();toast(input.value==='long'?'Длинная прокрутка включена':'Обычная прокрутка включена');
  });
  document.querySelectorAll('input[name="soundSetting"]').forEach(input=>input.onchange=()=>{
    sound=input.value==='on';localStorage.setItem('ul-sound',sound?'1':'0');applyAllSettingsV28();renderTop();toast(sound?'Звук включён':'Звук выключен');
  });
  const vol=document.querySelector('#soundVolume'); if(vol)vol.oninput=()=>{
    soundVolume=Math.max(0,Math.min(1,Number(vol.value)/100));localStorage.setItem('ul-sound-volume',String(soundVolume));if(sound)beep(330,.05,.14,'triangle');
  };
  const save=document.querySelector('#saveQuickSettings'); if(save)save.onclick=()=>{
    const ms=(document.querySelector('#quickMultipliers')?.value||'').split(',').map(x=>Number(x.trim().replace(',','.'))).filter(x=>Number.isFinite(x)&&x>1).slice(0,3);
    const ps=(document.querySelector('#quickPercents')?.value||'').split(',').map(x=>Number(x.trim().replace(',','.'))).filter(x=>Number.isFinite(x)&&x>0&&x<=MAX_CHANCE).slice(0,3);
    if(!ms.length||!ps.length)return toast('Укажи хотя бы один коэффициент и процент');
    localStorage.setItem('ul-quick-m',ms.join(','));localStorage.setItem('ul-quick-p',ps.join(','));rebuildQuickButtons();applyAllSettingsV28();toast('Быстрые кнопки сохранены');
  };
  const settings=document.querySelector('#themeQuick'); if(settings)settings.onclick=e=>{e.preventDefault();e.stopPropagation();applyAllSettingsV28();open('themeModal')};
}
...
function openChanceTargets(p){
  ...
  if(!grid)return;
  grid.innerHTML=list.length?list.map(({item,c})=>`<article class="skin-card chance-target-card" data-target-id="${esc(item.id)}"><div class="skin-img"><img src="${esc(item.image)}" alt=""></div><h3>${esc(item.name)}</h3><small>${esc(item.wear||item.categoryLabel||'Предмет')}</small><b>${fmt(item.priceUSD)}</b><span class="target-chance-badge">${c.toFixed(2)}%</span><button type="button" class="pick-target">Выбрать</button></article>`).join(''):`<div class="empty-state">В этом диапазоне пока нет подходящих предметов. Попробуй соседний процент.</div>`;
...
bindAllSettingsV28();applyAllSettingsV28();
