...
if(detected==='case'){
  // Cases vary enormously. Use a name-aware estimate instead of assigning every case $0.12.
  let caseBase=.35+((h%1000)/1000)*4.5;
  if(/operation bravo|esports 2013|weapon case(?! 2| 3)|hydra|shattered web|breakout/.test(n))caseBase=18+((h%1000)/1000)*55;
  else if(/souvenir package|dreamhack|katowice|cologne|stockholm|antwerp|paris/.test(n))caseBase=3+((h%1000)/1000)*24;
  else if(/capsule/.test(n))caseBase=.5+((h%1000)/1000)*7;
  else if(/gift package|pallet|parcel|present/.test(n))caseBase=2+((h%1000)/1000)*12;
  result=Math.round(caseBase*100)/100;
}
...
function sanePrice(apiPrice,name,category,wear,rarity){
  ...
  return {price:p,source:'рыночная цена'};
}
function detectCategory(rawCategory,name=''){
  const raw=String(rawCategory||'').toLowerCase();
  const text=(raw+' '+String(name||'')).toLowerCase();
  // The API source category has priority. A weapon skin can contain words such as
  // "Classic Crate" or the prefix "Souvenir" without being a case.
  ...
  // Explicit extra-file categories.
  if(raw==='case')return 'case';
  if(raw==='agent'||raw==='keychain'||raw==='sticker'||raw==='graffiti')return raw;
  return 'weapon';
}
function rarityOf(x){
...
async function loadPrices(){
  ...
  try{
    ...
    if(cached?.items){
      ...
      if(Date.now()-Number(cached.time||0)<PRICE_CACHE_TTL)return {map:stale,source:'кэш рыночных цен'};
    }
  }catch{}
  for(const url of PRICE_APIS){
    try{
      ...
      return {map:new Map(entries),source:'рыночные цены'};
    }catch{}
  }
  return {map:stale||new Map(),source:stale?'старый кэш рыночных цен':'резервная оценка'};
}
...
function normalizeExtraItem(x,i,forcedCategory,priceMap){
  ...
  return {id:`${forcedCategory}-${x.id||x.def_index||i}`,name,marketHash,wear:'',image,rarity,category,subtype:String(x.type||''),priceUSD:checked.price,priceSource:checked.source};
}
      ...
      raw.filter(x=>x.image&&x.name).forEach((x,i)=>{
        ...
        for(const wear of wears){
          ...
          for(const variant of variants){
            ...
            let checked=sanePrice(api,hash,category,wear,rarity);
            if(!api&&variant.kind!=='normal')checked={price:Math.max(.03,checked.price*variant.mul),source:'резервная оценка'};
            out.push({id:`${x.id||i}-${variant.kind}-${wear}`,name:displayName,marketHash:hash,wear,image:x.image,rarity,category,variant:variant.kind,priceUSD:checked.price,priceSource:checked.source});
          }
        }
      });
    }else{
...
const STEAM_CACHE_KEY='ul-steam-price-cache-v1';
function parseSteamMoney(text){
  const raw=String(text||'').replace(/\s/g,'').replace(/[^0-9.,]/g,'');if(!raw)return 0;
  const lastComma=raw.lastIndexOf(','),lastDot=raw.lastIndexOf('.');let normalized=raw;
  if(lastComma>lastDot)normalized=raw.replace(/\./g,'').replace(',','.');else normalized=raw.replace(/,/g,'');
...
async function fetchSteamPrice(item){
  ...
  const url='https://steamcommunity.com/market/priceoverview/?appid=730&currency=1&market_hash_name='+encodeURIComponent(item.marketHash);
  const urls=[url,'https://api.allorigins.win/raw?url='+encodeURIComponent(url),'https://api.codetabs.com/v1/proxy?quest='+encodeURIComponent(url),'https://corsproxy.io/?url='+encodeURIComponent(url)];
  try{for(const endpoint of urls){try{const r=await fetch(endpoint);if(!r.ok)continue;const data=await r.json();const price=parseSteamMoney(data.lowest_price||data.median_price);if(data.success&&price>0){cache[item.marketHash]={price,time:Date.now()};localStorage.setItem(STEAM_CACHE_KEY,JSON.stringify(cache));return price}}catch{}}}finally{steamPriceInFlight.delete(item.marketHash)}
  return null;
}
async function enrichSteamPrices(items){
  const todo=items.filter(x=>x&&x.priceSource!=='Steam Market').slice(0,12);let changed=false;
  for(let i=0;i<todo.length;i+=3){const batch=todo.slice(i,i+3);const prices=await Promise.all(batch.map(fetchSteamPrice));prices.forEach((p,j)=>{if(p>0){batch[j].priceUSD=p;batch[j].priceSource='Steam Market';changed=true}})}
