...
function setTheme(name){document.documentElement.dataset.theme=name;localStorage.setItem('ul-theme',name);$$('[data-theme]').forEach(b=>b.classList.toggle('active',b.dataset.theme===name))}
...
$$('.nav').forEach(b=>b.onclick=()=>showPage(b.dataset.page));$$('[data-close]').forEach(b=>b.onclick=()=>close(b.dataset.close));$('#currency').onchange=e=>{currency=e.target.value;localStorage.setItem('ul-currency',currency);renderAll();renderCatalog()};$('#search').oninput=()=>{shown=30;renderCatalog()};$('#category').onchange=renderCatalog;$('#sort').onchange=renderCatalog;$('#more').onclick=()=>{shown+=30;renderCatalog()};$('#favoriteOnly').onclick=()=>{$('#favoriteOnly').classList.toggle('active');shown=30;renderCatalog()};$('#clearTarget').onclick=()=>{target=null;lastOutcome=null;syncUpgrade()};$('#clearSource').onclick=()=>{sourceItems=[];selected.clear();cashDraft='';lastOutcome=null;syncUpgrade()};$('#sourceMode').onclick=()=>{sourceItems=[];selected.clear();cashDraft='';lastOutcome=null;syncUpgrade()};$('#chooseInventory').onclick=()=>{selected=new Set(sourceItems.map(inventoryKey));renderPicker();open('modal')};$('#applySources').onclick=()=>{const picked=inventory.filter(x=>selected.has(inventoryKey(x)));if(!picked.length)return toast('Сначала выбери хотя бы один скин');sourceItems=picked;cashDraft='';lastOutcome=null;target=null;close('modal');syncUpgrade();toast(`В ставку добавлено: ${picked.length}`)};$('#upgradeBtn').onclick=startUpgrade;$('#autoTarget').onclick=()=>pickByRatio(2);$$('.quick button').forEach(b=>b.onclick=()=>b.dataset.m?pickByRatio(+b.dataset.m):pickByChance(+b.dataset.p));$('#sellAll').onclick=()=>inventory.length?sellItems(inventory.map(inventoryKey)):toast('Инвентарь пуст');$('#soundQuick').onclick=()=>{sound=!sound;localStorage.setItem('ul-sound',sound?'1':'0');renderTop()};$('#fastQuick').onclick=()=>{fast=!fast;localStorage.setItem('ul-fast',fast?'1':'0');renderTop()};$('#saveNick').onclick=async()=>{try{const nickname=$('#nickInput').value.trim().slice(0,24)||'Игрок';await setDoc(doc(db,'profiles',user.uid),{nickname},{merge:true});toast('Ник сохранён')}catch(e){toast(humanError(e))}};
...
$('#themeQuick').onclick=()=>{setTheme(localStorage.getItem('ul-theme')||'classic');open('themeModal')};
$$('[data-theme]').forEach(b=>b.onclick=()=>{setTheme(b.dataset.theme);close('themeModal')});
