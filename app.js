...
function outcomeStakeUSD(){
  ...
  if(Array.isArray(lastOutcome.sources)&&lastOutcome.sources.length)return lastOutcome.sources.reduce((a,x)=>a+Number(x.priceUSD||0),0);
  return Number(lastOutcome.cashUSD||0);
}
function renderSource(){
  ...
  if(shownSources.length){
    ...
    el.innerHTML=`<div class="mini-source-grid">${visible.map(x=>`<div class="mini-source"><img src="${esc(x.image)}"><span>${esc(x.name)}</span></div>`).join('')}</div>${shownSources.length>4?`<small class="more-sources">+${shownSources.length-4} предметов</small>`:''}<b class="source-total">${fmt(sourceItems.length?stakeUSD():outcomeStakeUSD())}</b>`;
    return;
  }
...
function updateUpgradeMetrics(){
  ...
  btn.disabled=spin||!target||v<=0||tooCheap||c<=0||(!sourceItems.length&&v>Number(userDoc?.balanceUSD||0));
