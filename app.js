...
async function openSelectedCase(){if(!activeCase||caseQty<1||caseQty>10)return;const cost=activeCase.priceUSD*caseQty;if(Number(userDoc?.balanceUSD||0)<cost)return toast('Недостаточно виртуального баланса');const btn=$('#openCaseBtn');btn.disabled=true;const pool=casePool(activeCase);const drops=Array.from({length:caseQty},()=>weightedDrop(pool,activeCase));const reel=Array.from({length:28},(_,i)=>i===24?drops[0]:pool[Math.floor(Math.random()*pool.length)]);const old=$('#caseModalContent').innerHTML;$('#caseModalContent').innerHTML=`<h2>${esc(activeCase.name)}</h2><div class="case-spinner"><div id="caseSpinnerTrack" class="case-spinner-track">${reel.map(x=>`<div class="spin-item"><img src="${esc(x.image)}"><small>${esc(x.name)}</small></div>`).join('')}</div></div><p class="muted">Открываем ${caseQty} кейс(ов)…</p>`;requestAnimationFrame(()=>requestAnimationFrame(()=>{$('#caseSpinnerTrack').style.transform='translateX(calc(50% - 3260px))'}));await new Promise(r=>setTimeout(r,fast?700:3500));try{await runTransaction(db,async tx=>{const ref=doc(db,'users',user.uid),snap=await tx.get(ref),d=snap.data()||{},balance=Number(d.balanceUSD||0);if(balance<cost)throw new Error('Недостаточно виртуального баланса');const hist=Array.isArray(d.history)?d.history:[];const entry={id:crypto.randomUUID(),type:'case',caseName:activeCase.name,caseSlug:activeCase.slug,count:caseQty,costUSD:cost,drops:drops.map(x=>({name:x.name,image:x.image,priceUSD:x.priceUSD,rarity:x.rarity})),createdAt:Date.now()};tx.update(ref,{balanceUSD:balance-cost,history:[entry,...hist].slice(0,100)})});lastCaseDrops=drops.map(x=>({...x,selected:false,sold:false,kept:false}));close('caseModal');renderCaseResults();open('caseResultModal')}catch(e){toast(humanError(e));$('#caseModalContent').innerHTML=old}finally{btn.disabled=false}}
