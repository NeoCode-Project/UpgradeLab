...
function card(x,mode='catalog'){
  ...
  const id=String(mode==='catalog'?(x.id||x.marketHash):(x.uid||x.id||x.marketHash));
...
function renderInventory(){$('#inventoryGrid').innerHTML=inventory.map(x=>card(x,'inventory')).join('')||'<div class="empty-state">Инвентарь пуст</div>';$$('#inventoryGrid .skin-card').forEach(e=>{const x=inventory.find(x=>String(x.uid)===String(e.dataset.id));e.querySelector('.use').onclick=()=>{sourceItems=[x];lastOutcome=null;target=null;showPage('upgrade');syncUpgrade()};e.querySelector('.sell').onclick=()=>sellItems([x.uid])});wireFavoriteButtons('#inventoryGrid',inventory);renderPicker()}
function renderPicker(){
  ...
  $$('#modalGrid .skin-card').forEach(e=>{
    ...
    const toggle=()=>{selected.has(id)?selected.delete(id):selected.add(id);renderPicker()};
...
function syncUpgrade(){renderSource();renderTarget();const c=chance(),v=stakeUSD(),tooCheap=!!target&&v>0&&target.priceUSD<=v;$('#chance').textContent=(tooCheap?0:c).toFixed(2)+'%';$('#multi').textContent=target&&v>0?'x'+(target.priceUSD/v).toFixed(2):'x0.00';$('#winArc').style.setProperty('--chance',`${(tooCheap?0:c)*3.6}deg`);$('#upgradeBtn').disabled=spin||!target||v<=0||tooCheap||c<=0||(!sourceItems.length&&v>Number(userDoc?.balanceUSD||0));if(!lastOutcome)$('#result').textContent=!target?'Выбери целевой скин':tooCheap?'Целевой скин должен быть дороже ставки':c>=MAX_CHANCE?'Шанс ограничен 80%':'Готово к апгрейду'}
