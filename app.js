...
async function activatePromo(){
  ...
  try{
    ...
    if(xp||levels){
      batch.set(profileRef,{xp:Number(pd.xp||0)+xp,level:Math.max(1,Number(pd.level||1)+levels)},{merge:true});
    }
    await batch.commit();
    const rewardText=[money?fmt(money):'',xp?`+${xp} XP`:'',levels?`+${levels} ур.`:'',awarded.length?`${awarded.length} предмет(а)`:'' ].filter(Boolean).join(', ');
    $('#promoCode').value='';
