...
function setTheme(name,notify=true){
  ...
  $$('[data-theme]').forEach(b=>b.classList.toggle('active',b.dataset.theme===name));
...
function detectCategory(rawCategory,name=''){
  ...
  const knifeWords=/\b(knife|knives|bayonet|karambit|dagger|daggers|bowie|falchion|navaja|stiletto|talon|ursus|paracord|survival|skeleton|nomad|kukri|gut knife|flip knife|huntsman|classic knife)\b/;
...
async function loadCatalog(){
  ...
  raw.filter(x=>x.image&&x.name).forEach((x,i)=>{
    ...
    for(const wear of wears){
      ...
      const category=detectCategory(rawCategory,baseName);
