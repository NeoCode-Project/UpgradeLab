...
function marketPriceFor(priceMap, marketHash){
  const direct=Number(priceMap.get(marketHash)||0);
  if(direct>0)return direct;
  // Some markets omit the star spacing or trademark symbol.
  const aliases=[
    marketHash.replace('StatTrak™','StatTrak'),
    marketHash.replace('★ ','★'),
    marketHash.replace('★','★ '),
  ];
  for(const alias of aliases){const p=Number(priceMap.get(alias)||0);if(p>0)return p}
  return 0;
}
...
function normalizeWeaponItem(x,i,priceMap=new Map()){
  const marketHash=String(x.market_hash_name||x.name||'').trim();
  const image=x.image||'';
...
function applyBulkPrices(priceResult){
  const map=priceResult?.map||new Map();let found=0;
  for(const item of skins){
    const p=marketPriceFor(map,item.marketHash);
    if(p>0){item.priceUSD=p;item.priceSource=`${priceResult.source} — реальная цена`;found++}
    else if(!(Number(item.priceUSD)>0)){item.priceUSD=null;item.priceSource='Цена временно недоступна'}
  }
  ...
  $('#status').textContent=`${skins.length} предметов: оружие ${counts.weapon+counts.knife+counts.glove}, агенты ${counts.agent}, брелоки ${counts.keychain}, наклейки ${counts.sticker}, граффити ${counts.graffiti}, кейсы ${counts.case}. Реальные цены найдены для ${found} предметов. Источник: ${priceResult.source}.`;
  renderCatalog();
}
...
const STEAM_CACHE_KEY='ul-steam-price-cache-v35';
