...
function openCase(c){activeCase=c;caseQty=1;const pool=casePool(c),preview=pool.slice().sort(()=>Math.random()-.5).slice(0,12);$('#caseModalContent').innerHTML=`<div class="case-open-layout"><div class="case-open-cover"><img src="${c.image}" alt="${esc(c.name)}"></div><div class="case-open-info"><span class="eyebrow">${c.category.toUpperCase()}</span><h2>${esc(c.name)}</h2><p class="muted">Выбери количество кейсов. Максимум — 10 за одно открытие.</p><div class="case-open-price">${fmt(c.priceUSD)} за кейс</div><div class="case-qty">${[1,2,3,5,10].map(n=>`<button class="ghost ${n===1?'active':''}" data-case-qty="${n}">×${n}</button>`).join('')}</div><div class="case-open-summary"><span>Итого</span><b id="caseOpenTotal">${fmt(c.priceUSD)}</b></div><button id="openCaseBtn" class="upgrade-btn">Открыть ×1</button></div></div><div class="case-contents"><h3>Что может выпасть</h3><div class="case-contents-grid">${preview.map(x=>`<div class="case-content-item"><img src="${esc(x.image)}"><b>${esc(x.name)}</b><small>${fmt(x.priceUSD)}</small></div>`).join('')}</div></div>`;$$('[data-case-qty]').forEach(b=>b.onclick=()=>{caseQty=Number(b.dataset.caseQty);$$('[data-case-qty]').forEach(x=>x.classList.toggle('active',x===b));$('#caseOpenTotal').textContent=fmt(c.priceUSD*caseQty);$('#openCaseBtn').textContent=`Открыть ×${caseQty}`});$('#openCaseBtn').onclick=openSelectedCase;open('caseModal')}
function weightedDrop(pool,c){const target=Math.random(),mult=target<.003?25:target<.02?8:target<.1?3:target<.35?1.2:.45;const wanted=c.priceUSD*mult,cands=pool.slice().sort((a,b)=>Math.abs(a.priceUSD-wanted)-Math.abs(b.priceUSD-wanted));return {...cands[Math.floor(Math.random()*Math.min(5,cands.length))],uid:crypto.randomUUID(),obtainedAt:Date.now(),fromCase:c.name}}
...
async function animateCaseDrop(pool,drop,index,total){const reel=buildCaseReel(pool,drop),targetIndex=26;$('#caseModalContent').innerHTML=`<div class="case-spin-head"><div><span class="eyebrow">ОТКРЫТИЕ КЕЙСОВ</span><h2>${esc(activeCase.name)}</h2></div><b>${index+1} / ${total}</b></div><div class="case-spinner"><div id="caseSpinnerTrack" class="case-spinner-track">${reel.map((x,i)=>`<div class="spin-item ${i===targetIndex?'spin-winner':''}"><img src="${esc(x.image)}"><small>${esc(x.name)}</small><b>${fmt(x.priceUSD)}</b></div>`).join('')}</div></div><div id="caseSpinResult" class="case-spin-result hidden"><img src="${esc(drop.image)}"><div><small>Выпало</small><strong>${esc(drop.name)}</strong><b>${fmt(drop.priceUSD)}</b></div></div><p class="muted case-spin-status">Кейс ${index+1} из ${total} — рулетка крутится…</p>`;const spinner=$('.case-spinner'),track=$('#caseSpinnerTrack');await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));const target=track.children[targetIndex];const spinnerCenter=spinner.clientWidth/2;const targetCenter=target.offsetLeft+target.offsetWidth/2;const jitter=(Math.random()-.5)*Math.min(34,target.offsetWidth*.28);track.style.transform=`translateX(${spinnerCenter-targetCenter+jitter}px)`;await new Promise(r=>setTimeout(r,fast?420:2600));target.classList.add('landed');$('#caseSpinResult').classList.remove('hidden');$('.case-spin-status').textContent=`Кейс ${index+1} из ${total} — выпало: ${drop.name}`;await new Promise(r=>setTimeout(r,fast?180:650))}
async function openSelectedCase(){if(!activeCase||caseQty<1||caseQty>10)return;const cost=activeCase.priceUSD*caseQty;if(Number(userDoc?.balanceUSD||0)<cost)return toast('Недостаточно виртуального баланса');const btn=$('#openCaseBtn');btn.disabled=true;const pool=casePool(activeCase);const drops=Array.from({length:caseQty},()=>weightedDrop(pool,activeCase));const old=$('#caseModalContent').innerHTML;try{for(let i=0;i<drops.length;i++)await animateCaseDrop(pool,drops[i],i,drops.length);await runTransaction(db,async tx=>{const ref=doc(db,'users',user.uid),snap=await tx.get(ref),d=snap.data()||{},balance=Number(d.balanceUSD||0);if(balance<cost)throw new Error('Недостаточно виртуального баланса');const hist=Array.isArray(d.history)?d.history:[];const entry={id:crypto.randomUUID(),type:'case',caseName:activeCase.name,caseSlug:activeCase.slug,count:caseQty,costUSD:cost,drops:drops.map(x=>({name:x.name,image:x.image,priceUSD:x.priceUSD,rarity:x.rarity})),createdAt:Date.now()};tx.update(ref,{balanceUSD:balance-cost,history:[entry,...hist].slice(0,100)})});lastCaseDrops=drops.map(x=>({...x,selected:false,sold:false,kept:false}));close('caseModal');renderCaseResults();open('caseResultModal')}catch(e){toast(humanError(e));$('#caseModalContent').innerHTML=old;const retry=$('#openCaseBtn');if(retry)retry.onclick=openSelectedCase}finally{btn.disabled=false}}
function renderCaseResults(){const grid=$('#caseResultGrid');if(!grid)return;$('#caseResultTotal').textContent=`Общая стоимость: ${fmt(lastCaseDrops.filter(x=>!x.sold).reduce((a,x)=>a+Number(x.priceUSD||0),0))}`;grid.innerHTML=lastCaseDrops.map((x,i)=>`<article class="drop-card ${x.selected?'selected':''} ${x.sold?'sold':''}" data-drop="${i}"><input class="drop-select" type="checkbox" ${x.selected?'checked':''} ${x.sold?'disabled':''}><img src="${esc(x.image)}"><h4>${esc(x.name)}</h4><b>${fmt(x.priceUSD)}</b><div class="drop-card-actions"><button class="ghost sell-drop" ${x.sold?'disabled':''}>${x.sold?'Продано':'Продать'}</button></div></article>`).join('');$$('.drop-card').forEach(e=>{const i=Number(e.dataset.drop),x=lastCaseDrops[i];e.querySelector('.drop-select').onchange=ev=>{x.selected=ev.target.checked;renderCaseResults()};e.querySelector('.sell-drop').onclick=()=>sellCaseDrops([i])})}
...
async function keepCaseDrops(){const items=lastCaseDrops.filter(x=>!x.sold&&!x.kept);if(!items.length)return toast('Все предметы уже обработаны');try{await runTransaction(db,async tx=>{const ref=doc(db,'users',user.uid),s=await tx.get(ref),d=s.data()||{},inv=Array.isArray(d.inventory)?d.inventory:[];tx.update(ref,{inventory:[...items.map(({selected,sold,kept,...x})=>x),...inv]})});items.forEach(x=>x.kept=true);close('caseResultModal');toast(`${items.length} предметов добавлено в инвентарь`)}catch(e){toast(humanError(e))}}
